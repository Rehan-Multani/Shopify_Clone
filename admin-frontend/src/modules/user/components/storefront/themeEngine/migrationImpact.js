/**
 * Wave 5 — section impact + field remapping (FE mirror).
 */
...
export const FIELD_MAPPINGS = [
    ...
    },
    {
        ...
        to: 'Testimonials',
        toType: 'testimonials',
        fieldMap: { title: 'title', heading: 'title' },
    },
    {
        ...
        fieldMap: { title: 'title', heading: 'title', limit: 'limit', columns: 'columns' },
        transforms: {
            columns: (value) => {
    ...
    },
    {
        ...
        to: 'ProductGrid',
        toType: 'featured-products',
        fieldMap: { title: 'title', columns: 'columns' },
        transforms: {
            columns: (value) => {
                if (value && typeof value === 'object') return value;
                const n = Number(value) || 4;
...
export const IMPACT = { SAFE: 'SAFE', WARNING: 'WARNING', REQUIRES_ACTION: 'REQUIRES_ACTION' };
...
export const findFieldMapping = (section = {}) => {
    ...
    return FIELD_MAPPINGS.find(
        (m) => norm(m.fromType) === type || norm(m.from) === component
    ) || null;
};
...
export const buildUpgradeImpactReport = ({
    ...
    for (const sec of sections || []) {
        ...
        if (unsupported) {
            ...
            if (mapping) notes.push(`${mapping.from || sec.component} → ${mapping.to || mapping.toType}`);
            else notes.push('No safe remap suggestion');
        } else if (mapping?.transforms && Object.keys(mapping.transforms).length) {
            ...
            if (compareSemver(toVersion, fromVersion) > 0
                && (sec.type === 'hero' || sec.type === 'featured-products')) {
...
export const remapSectionWithFields = (original, suggestion) => {
    ...
    const mapping = findFieldMapping(original)
        || FIELD_MAPPINGS.find((m) => norm(m.toType) === norm(suggestion.type));
