...
const CustomersTab = () => {
    ...
    const [activeView, setActiveView] = useState('customers'); // 'customers' or 'subscribers'
    const [loading, setLoading] = useState(true);
    const [subscribersLoading, setSubscribersLoading] = useState(false);
    const [search, setSearch] = useState('');
    const [deleteModal, setDeleteModal] = useState({ open: false, id: null, name: '' });
    const [deleteSubscriberModal, setDeleteSubscriberModal] = useState({ open: false, id: null, email: '' });
    ...
    const fetchSubscribers = async () => {
        ...
        try {
            const storeId = localStorage.getItem('activeStoreId') || '';
            const res = await fetch(`${API_URL}/customers/subscribers`, {
                headers: { 'Authorization': `Bearer ${token}`, 'x-store-id': storeId }
            });
            const data = await res.json();
    ...
    const handleDeleteSubscriber = async () => {
        try {
            const storeId = localStorage.getItem('activeStoreId') || '';
            const res = await fetch(`${API_URL}/customers/subscribers/${deleteSubscriberModal.id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}`, 'x-store-id': storeId }
            });
            if (res.ok) {
                setSubscribers(prev => prev.filter(s => s._id !== deleteSubscriberModal.id));
        ...
        setDeleteSubscriberModal({ open: false, id: null, email: '' });
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-[#5c5f62] mt-1">
            ...
            <div className="flex border-b border-gray-200">
                <button
                    onClick={() => { setActiveView('customers'); setSearch(''); }}
                ...
                <button
                    onClick={() => { setActiveView('subscribers'); setSearch(''); }}
                    className={`py-3 px-6 text-sm font-bold border-b-2 transition-all ${
            ...
            {activeView === 'customers' ? (
                ...
                subscribersLoading ? (
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
                        <div className="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            <svg className="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 19v-8.93a2 2 0 01.89-1.664l8-5.333a2 2 0 012.22 0l8 5.333A2 2 0 0121 10.07V19M3 19a2 2 0 002 2h14a2 2 0 002-2M3 19l6.75-4.5M21 19l-6.75-4.5M3 10l6.75 4.5M21 10l-6.75 4.5m0 0l-5.625-3.75" />
                            </svg>
                        </div>
                        <h3 className="font-bold text-[#202223] mb-1">No subscribers found</h3>
                        <p className="text-sm text-[#5c5f62] mb-4">Newsletter subscriber sign-ups from your storefront will appear here</p>
                    </div>
                ) : (
                    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm animate-in fade-in duration-300">
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-gray-100">
                                        <th className="text-left text-[10px] font-black text-gray-500 tracking-[0.15em] uppercase px-5 py-3">Email Address</th>
                                        <th className="text-left text-[10px] font-black text-gray-500 tracking-[0.15em] uppercase px-5 py-3">Subscribed Date</th>
                                        <th className="text-right text-[10px] font-black text-gray-500 tracking-[0.15em] uppercase px-5 py-3">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredSubscribers.map((subscriber, idx) => (
                                        <tr key={subscriber._id} className={`group hover:bg-gray-50/80 transition-colors ${idx !== filteredSubscribers.length - 1 ? 'border-b border-gray-50' : ''}`}>
                                            <td className="px-5 py-4">
                                                <span className="font-bold text-sm text-[#202223]">{subscriber.email}</span>
                                            </td>
                                            <td className="px-5 py-4">
                                                <span className="text-sm text-[#5c5f62]">{new Date(subscriber.createdAt).toLocaleDateString(undefined, { dateStyle: 'medium' })}</span>
                                            </td>
                                            <td className="px-5 py-4">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    <button
                                                        onClick={() => setDeleteSubscriberModal({ open: true, id: subscriber._id, email: subscriber.email })}
                                                        className="p-2 hover:bg-red-50 rounded-lg transition-all text-gray-400 hover:text-red-500"
            ...
            {deleteSubscriberModal.open && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setDeleteSubscriberModal({ open: false, id: null, email: '' })}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 space-y-4" onClick={e => e.stopPropagation()}>
                        <div className="w-12 h-12 bg-red-100 rounded-xl flex items-center justify-center mx-auto">
                            <svg className="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        ...
                        <div className="text-center">
                            <h3 className="font-bold text-lg text-[#202223]">Delete Subscriber</h3>
                            <p className="text-sm text-[#5c5f62] mt-1">Are you sure you want to unsubscribe <strong>{deleteSubscriberModal.email}</strong>? This action cannot be undone.</p>
                        </div>
                        <div className="flex gap-3">
                            <button onClick={() => setDeleteSubscriberModal({ open: false, id: null, email: '' })} className="flex-1 px-4 py-2.5 bg-gray-100 text-[#202223] rounded-lg font-bold text-sm hover:bg-gray-200 transition-all">
                                Cancel
                            </button>
                            <button onClick={handleDeleteSubscriber} className="flex-1 px-4 py-2.5 bg-red-600 text-white rounded-lg font-bold text-sm hover:bg-red-700 transition-all">
                                Delete
                            </button>
                        </div>
                    </div>
