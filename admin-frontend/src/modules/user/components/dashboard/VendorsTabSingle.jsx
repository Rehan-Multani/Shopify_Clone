                                    ...
                                    <tr key={v._id} className="hover:bg-gray-50/30 transition-colors">
                                        <td className="px-5 py-3.5">
                                            <div className="flex items-center gap-3">
                                                {v.logo ? (
                                                    <img 
                                                        src={v.logo.startsWith('http') ? v.logo : `${CATALOG_API_URL.replace('/api', '')}${v.logo}`} 
                                                        alt={v.name} 
                                                        className="w-9 h-9 rounded-full object-cover border border-gray-200" 
                                                    />
                                                ) : (
                                                    <div className="w-9 h-9 rounded-full bg-zinc-100 flex items-center justify-center text-xs font-bold text-zinc-650 border border-gray-200">
                                                        {v.name?.charAt(0).toUpperCase() || 'V'}
                                                    </div>
                                                )}
                                                <div>
                                                    <span 
                                                        onClick={() => navigate(`/dashboard/vendors/view/${v._id}`)} 
                                                        className="text-sm font-bold text-[#202223] hover:text-[#008060] hover:underline cursor-pointer transition-all"
                                                    >
                                                        {v.name}
                                                    </span>
                                                    <p className="text-[11px] text-[#9CA3AF] mt-0.5">{v.email}</p>
                                                </div>
                                            </div>
                                        </td>
