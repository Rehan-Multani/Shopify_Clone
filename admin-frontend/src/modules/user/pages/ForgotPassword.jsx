...
/**
 * Shared forgot/reset password UI for merchant + vendor.
 * /forgot-password → merchant
 * /vendor/forgot-password → vendor
 */
const ForgotPassword = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const isVendor = location.pathname.startsWith('/vendor');
  const role = isVendor ? 'vendor' : 'merchant';
  const loginPath = isVendor ? '/vendor/login' : '/admin/login';
  const audienceLabel = isVendor ? 'Vendor' : 'Merchant';
  ...
  const handleRequestOtp = async (e) => {
    ...
    try {
      const response = await fetch(`${AUTH_API_URL}/${role}/forgot-password`, {
        method: 'POST',
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const response = await fetch(`${AUTH_API_URL}/${role}/verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const response = await fetch(`${AUTH_API_URL}/${role}/reset-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center px-6 py-10 ${isVendor ? 'bg-[#0f172a]' : 'bg-[#111827]'}`}>
      <div className="mb-8 flex items-center gap-3">
        ...
        {isVendor && (
          <span className="bg-teal-500/10 text-teal-400 border border-teal-500/20 px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wider">
            Vendor
          </span>
        )}
      </div>

      <div className={`w-full max-w-[440px] border border-white/10 rounded-[40px] shadow-[0_8px_40px_rgba(0,0,0,0.5)] p-8 space-y-5 ${isVendor ? 'bg-[#1e293b]' : 'bg-[#1F2937]'}`}>
        <div className="space-y-1">
          <h1 className="text-3xl font-bold text-white tracking-tight">
            {step === 1 ? 'Recover Password' : step === 2 ? 'Verify OTP' : 'Set New Password'}
          </h1>
          <p className="text-gray-400 text-[15px] font-medium">
            {step === 1 && `Enter your registered ${audienceLabel.toLowerCase()} email to receive a verification code`}
        ...
        {step === 2 && (
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-[13px] font-semibold text-gray-300 block ml-0.5">Verification Code (OTP)</label>
              <input
                type="text"
                required
                value={otp}
        ...
        {step === 3 && (
          <form onSubmit={handleResetPassword} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-[13px] font-semibold text-gray-300 block ml-0.5">New Password</label>
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                />
                <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white transition-colors" aria-label={showNewPassword ? 'Hide password' : 'Show password'}>
                  {showNewPassword ? (
            ...
            <div className="space-y-1.5">
              <label className="text-[13px] font-semibold text-gray-300 block ml-0.5">Confirm Password</label>
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                />
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white transition-colors" aria-label={showConfirmPassword ? 'Hide password' : 'Show password'}>
                  {showConfirmPassword ? (
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                  ) : (
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" /></svg>
        ...
        <div className="text-center pt-2">
          <p className="text-gray-400 text-[14px]">
            ...
            <Link to={loginPath} className="text-storify font-semibold hover:underline">
              Back to Login
            </Link>
          </p>
        </div>
