/**
 * Theme versioning + migration foundation (backend).
 * Never auto-run on public storefront reads.
 */
...
/**
 * Explicit migrations keyed by theme folder.
 * Each step: { from, to, changelog: string[], migrate(config) => next }
 */
...
/**
 * Walk versions step-by-step. Does not jump when intermediate migrations are missing.
 */
export const migrateThemeConfig = (config, themeFolder, fromVersion, toVersion) => {
    ...
    if (!steps.length) {
        // Metadata-only bump when no migration steps defined
    ...
    // Ensure contiguous path: first step.from must equal fromVersion
    if (compareSemver(steps[0].from, fromVersion) !== 0) {
        return {
            ok: false,
            message: `No migration path from ${fromVersion} (next available: ${steps[0].from})`,
            config,
            applied: [],
            changelog: [],
        };
    ...
    for (const step of steps) {
        if (compareSemver(step.from, cursor) !== 0) {
            return {
                ok: false,
                message: `Broken migration chain at ${cursor} → expected from ${step.from}`,
                config: next,
                applied,
                changelog,
            };
